...
A flat task list says what to do. It usually does not prove that the work composes into the intended outcome.
...
The canonical plan lives in `.pyramid/plan.json`. Runtime state and immutable events live beside it. Graph snapshots, ready indexes, Markdown, and HTML are generated projections—not mutation interfaces.
...
An implementation is not verified merely because an agent marked it implemented. An intent is not complete until its evidence exists, its required branches and gates pass, and the plan is formally closed.
